...
*What if the "singularity" isn't a discontinuity in capability growth, but a discontinuity in our ability to measure it?*

![Optimization Power Curve](https://i.ibb.co/zVgBjZH/Power-Difference.png)

*Figure 1: A measurement envelope, not a capability curve. The plot shows how a binary success rate $$s$$ (a win-rate / accept-reject metric) can become an ill-conditioned "ruler" for the underlying optimization-power gap $$\Delta P$$: near $$s \approx 0$$ or $$s \approx 1$$, many very different $$\Delta P$$ values are observationally consistent with almost the same telemetry.*

## Introduction

People often talk about "hard vs soft takeoff" as if we're debating the *shape of capability growth*. This post is narrower: it's about the **shape of what we can measure**.

The reframing is: the "singularity" can be treated as a psychometric failure mode—an increasing divergence between an **objective proxy** (instrumental convergence / delegation win-rate) and a **latent construct** (optimization power). We observe the former because it's easy (e.g., accept vs retry), but we care about the latter because it's what governs how much an agent can optimize across objectives.

Concretely, we:

- Define an objective **optimality probability** $$p_\Sigma(B \ge A)$$ (a win-rate over tasks/objectives) and a latent **$$\Sigma$$-power** $$\mathcal{C}_\Sigma(\cdot)$$.
- Use a simple concentration assumption on reward gaps to relate upset probability to the **power gap** $$\Delta P$$.
- Show why binary success rates saturate: at the tails, the same accept/reject telemetry is compatible with a wide range of latent power gaps.

The takeaway is an observability constraint: near the extremes, you should avoid inferring sharp discontinuities (or ruling them out) from success-rate curves alone—the "ruler" breaks before the world necessarily does.
...
The key implication is about **detectability limits**: because our main relationship is a *one-sided tail bound* (an upper bound on the upset probability as a function of the power gap under sub-Gaussian noise), it fundamentally constrains what kinds of takeoff we can even *observe* from accept/reject telemetry.

Concretely, the curve should be read as: **when a binary success-rate metric saturates, it stops being a reliable ruler for distinguishing "smooth" vs "sharp" changes in underlying capability.** At the extremes, many very different underlying trajectories are observationally consistent with essentially the same telemetry.

1. **Low success rates ($$s \approx 0$$)**  
   The bound permits very large negative $$\Delta P$$. So "the model usually loses to the outside option" does *not* tightly identify *how much worse* it is—especially once retry policies, patience, and best-of-$$k$$ behavior vary across tasks.
...
3. **High success rates ($$s \approx 1$$)**  
   The envelope also permits very large positive $$\Delta P$$, but the correct interpretation is **not** "we've demonstrated a huge gap." It's that the *metric has saturated*: once success is near-certain, additional capability gains may produce little movement in $$s$$, and conversely small changes in $$s$$ can be compatible with a wide range of underlying gaps. This can make progress look "sharp" or "flat" depending on how you transform the metric, even when underlying capability growth is smooth.

**Takeoff-relevant takeaway:** upper-bound–based reasoning plus saturated accept/reject telemetry imposes a hard observational constraint. Near the extremes, the same data can be consistent with very different "takeoff shapes," so you should avoid inferring sharp discontinuities (or ruling them out) from success-rate curves alone.

## Conclusion

This analysis is best viewed as a caution about **what telemetry can resolve**, not as direct evidence for a particular takeoff regime. Under the stated assumptions, a binary delegation success rate $$s$$ becomes an ill-conditioned instrument near 0 and 1, which limits our ability to detect whether capability changes are smooth or sharp using that signal alone.

As AI systems improve, telemetry can still be useful—especially in the mid-range where it has more resolution—but interpreting "takeoff" from saturated accept/reject metrics requires either (i) richer measurements (graded outcomes, calibrated scoring, task-level difficulty controls) or (ii) explicit modeling of retries, thresholds, and task-mix drift. Without that, the data may be compatible with multiple takeoff narratives.
