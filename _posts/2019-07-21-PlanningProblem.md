...
Does this mean this idea failed? Not exactly. Recall the idealized model of planning. When we make plans we oftentimes will create tasks that themselves require further planning. It seems perfectly reasonable that we could simply "call" our meta-plan on these sub-tasks in order to reduce the effect of human bias. While I was overly optimistic in thinking about how long it would take to format the math the problem could've been heavily mitigated if I had a plan that made the task of making the mathematical section a sub-plan. If I had done this, I would've had a chance to look a the bigger picture and most likely would've proceeded differently. 

Ultimately, I think that the idea was a success because it was developed to enough generality to be useful in addressing these complications. By allowing the efficacy of planning to be a general stochastic process, failure of any particular model is much more the result of questionable idealization. The downside is that it isn't at all clear what the proper model for the efficacy of planning is. However, what I have learned is enough to say this. First, what I am currently engaging in is the construction of a plan that when executed can manage the planning of plans. Second, we're overly optimistic about how much time our plans will take to execute. Third, when we realize that our plans were too optimistic we are more than capable of updating them because we naturally make hierachical or tree like plans. Fourth, using these three observations I conclude that the current model I've developed is sufficient to fine tune itself. There is no need to continue worrying about abstract details at the meta-level. I need to practice using what I came up with.
