...
Same as the above, but with the level of formality necessary to make a rigorous argument. Let's say you have to complete some task \\(T\\). Suppose also that you have
...
\\(\Phi\\) that will produce an estimate of you how long it will take to execute \\(\pi\\).
...
It's worth noting a connection between this simplistic model and the
...
that the execution time of a task is reduced proportionally to the number of leaves
in the resulting tree. 

## b)

There is an important quantity here that can be used to make a practical algorithm.
...
should be constructed. Assume that this decision rule is
...
reasoning is that for planning to be an effective strategy the
minimum must be reached sometime before the estimated time for
execution of the initial plan. If this assumption is violated our original hypothesis would be as
...
our optima at \\(t'\\) is better than \\(t+\Phi(t)\\) for \\(t \in [0,t']\\).
